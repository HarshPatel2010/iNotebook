...
router.post("/login",
  ...
  // callbackk function for api
  async (req, res) => {
    let success = false;
    //if there are errors,return request and errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
    // destructuring of email password which are added 
    const { email, password } = req.body;
    try {
      // check whether user's email is exist or not
